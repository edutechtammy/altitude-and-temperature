...
class AtmosphericTemperatureChart {
    constructor() {
        ...
        // Chart dimensions
        this.margin = { top: 50, right: 260, bottom: 80, left: 100 };
        this.width = 600;
        this.height = 500;
    ...
    drawAtmosphericLayers() {
        ...
        this.atmosphericData.layers.forEach((layer, index) => {
            ...
            // Layer label
            const labelX = this.margin.left + this.width + 10;
            ...
            // Characteristics text (split into lines on sentence boundaries)
            const sentences = layer.characteristics.split('. ').map((s, i, arr) =>
                i < arr.length - 1 ? s + '.' : s
            );
            sentences.forEach((sentence, i) => {
                const charText = document.createElementNS('http://www.w3.org/2000/svg', 'text');
                charText.setAttribute('class', 'layer-characteristics');
                charText.setAttribute('x', labelX);
                charText.setAttribute('y', labelY + 8 + (i * 13));
                charText.setAttribute('dominant-baseline', 'middle');
                charText.textContent = sentence;
                layersGroup.appendChild(charText);
            });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const accordionContent = document.getElementById('accordion-table-content');
